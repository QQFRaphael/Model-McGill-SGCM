cc
cc ifort -cm -w -r8 -O3 -tpp7 -axW -xW -o specanSON specanSON.f -L /aos/home/hlin/igcm/lib -labort -laux -lblas -lfft -lutil
...
C                                                                          PARAM1.4     
      PARAMETER(NN=31,MM=31,NHEM=2,NL=10,MOCT=1,MG=128,JG=32,NWJ2=256        G100.1     
...
C                                                                          GRIDP.5     
     ...
     *              ,TG(IGC,NL),SPG(IGC),VGT(IGC,NL),UGT(IGC,NL)
C                                                                          LEGAU.2     
...
C                                                                          BALAN.8     
      LOGICAL LWTG,LGTW
      REAL UUG(MG,JGG,NL),VVG(MG,JGG,NL),TTG(MG,JGG,NL),SPSPG(MG,JGG)
      DIMENSION DAG(3*IGD+IGC)
      EQUIVALENCE (DAG(1),UG(1,1))
...
C     LGTW=.FALSE.
	...
	open(9,file=
     * 'ZDTP.dat.spec',form='unformatted')

cc     * ,access='append')

        print*,'ndays=  ',ndays, iyear
...
C*****read in spectral data
      ...
      READ(13)RKOUNT,RNTAPE,DAY,Z,D,T,SP,RMTAPE
      PRINT*,RKOUNT,RNTAPE,DAY,Z(1),RMTAPE

C*****subtract planetary vorticity
...
C*****read in grid point data
      ...
      READ(13)IYRMODYTM,UUG,VVG,TTG,SPSPG,IYRMODYTM2
      PRINT*,IYRMODYTM,IYRMODYTM2
...
C*****and set T0 for temperature
      ...
      DO 34 J=1,JGG
      DO 34 I=1,MG
      UUG(I,J,L)=UUG(I,J,L)/CV * CS(J)
      VVG(I,J,L)=VVG(I,J,L)/CV * CS(J)
 34   TTG(I,J,L)=(TTG(I,J,L) - 250.)/CT
...
C        inverse Legendre and Fourier transforms
 ...
 10      CONTINUE
     ...
     &              ,MGPP,MG,NRST,1)
         ...
         CALL SHUFFLE(JH,UG,UUG,NL)
         CALL SHUFFLE(JH,VG,VVG,NL)
...
C        direct Legendre and Fourier transforms
         ...
         CALL SLICE(JH,UG,UUG,NL)
         CALL SLICE(JH,VG,VVG,NL)
...
C*****and set temperature in centigrade
      ...
      UUG(I,J,L)=UUG(I,J,L)*CV / CS(J)
      VVG(I,J,L)=VVG(I,J,L)*CV / CS(J)
 35   TTG(I,J,L)=TTG(I,J,L)*CT + 250. - 273.15

C*****output
C     WRITE(9)0,UUG,VVG,TTG,SPSPG,0
      CALL WRTLVLS(UUG,NL)
      CALL WRTLVLS(VVG,NL)
      CALL WRTLVLS(TTG,NL)
      WRITE(9)SPSPG
...
c      DAY=RKOUNT/24.
	DAY=0.
      RNTAPE=100.
      WRITE(9)RKOUNT,RNTAPE,DAY,Z,D,T,SP,RNTAPE
...
C                                                                          GRIDP.5     
     ...
     *              ,TG(IGD),SPG(IGC),VGT(IGD),UGT(IGD)
C                                                                          COMFFT.2     
      COMMON/BATS/  BM1(IDE),AK(NNP),AQ(NL2),G(NL2),TAU(NL2)               BATS.6     
     +              ,KOUNT,KITS,KTOTAL,KRUN,BEGDAY,ITSPD,PNU21             BATS.7     
     +              ,DELT,DELT2,CV,CG,CT,PNU,PNU2                          BATS.8     
...
C                                                                          GRIDP.5     
     ...
     *              ,TG(IGL,NL),SPG(IGL),VGT(IGL,NL),UGT(IGL,NL)
      COMPLEX CHIG,SFG,UG,VG,TG,SPG,VGT,UGT
...
C     - make copies
      ...
      DO 10 L=1,NL                                                         LTDUV.31    
         DO 10 I=1,IGL                                                     LTDUV.32    
            VGT(I,L)=VG(I,L)                                             LTDUV.33    
            UGT(I,L)=-UG(I,L)                                            LTDUV.34    
            UG(I,L)=CMPA(I)*UG(I,L)/CSSQ(JH)                             LTDUV.35    
            VG(I,L)=CMPA(I)*VG(I,L)/CSSQ(JH)                             LTDUV.36    
   10 CONTINUE                                                             LTDUV.37    
                                                                           LTDUV.38    
C     legendre transfroms to give Z and D                                LTDUV.39    
                                                                           LTDUV.40    
         CALL HANAL(VG,GWORK,Z,NL,1)                                     LTDUV.41    
         CALL HANAL(UGT,GWORK,Z,NL,3)                                    LTDUV.42    
         CALL HANAL(UG,GWORK,D,NL,2)                                     LTDUV.43    
         CALL HANAL(VGT,GWORK,D,NL,4)                                    LTDUV.44    
                                                                           LTDUV.45    

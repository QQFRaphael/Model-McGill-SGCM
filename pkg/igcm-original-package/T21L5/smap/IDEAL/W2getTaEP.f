c11c W flux is defined by Takaya and Nakamura (1997, GRL)
        ...
        real olr(72,37),UA(51,4),UB(51,4),um(L+1,M),vm(L,M)
        real x(L,M),fll(73,37),zm(L,M),tf(73,37,nm),z9(73,37)
        real z(NLON,NLAT),z1(NLON,NLAT),zs(L,M)
        real z8(L,M),y1(nm),y2(nm),z500(L+1,M),yt(L,M,1,nm)
        real psi_x(L,M),psi_y(L,M),ex(L,M),ey(L,M)
        real psi_xx(L,M),psi_xy(L,M),psi_yy(L,M)
        real flla(73,37),st(L+1,M,nm)
        real pvt(L+1,M),z19(L,M,nm)
        REAL dx(m),phi(m),psi(L,M),
     2 cor(m),rcos(m),rsin(m),r2sin(m)
        DATA PHI1,PHI2,DELAM/20.,90.,5.0/

      open(25,file=
     * 'Zave.dat',form='unformatted',status='old')
      open(26,file=
     * 'Zanom.dat',form='unformatted',status='old')

c11c W flux is defined by Takaya and Nakamura (1997, GRL)
        PARAMETER ( mg=72,jg2=37,nl1=7,nl2=5,nm=18, L=72,M=17)
        parameter(NLON=96,NLAT=48)
        real um(L,M),vm(L,M)
        real x(L,M),zm(L,M)
        real z(NLON,NLAT),z1(NLON,NLAT),zs(L,M)
        real z8(L,M),y1(nm),y2(nm),z500(L,M),yt(L,M,1,nm)
...
c
      do 72 j = 1,m
        f0=cor(j)
        if(j.le.17.and.j.ge.15)f0=cor(15)
      jp1 = j + 1
      jm1 = j - 1
      if (j.eq.1) jm1 = 1
      if (j.eq.m) jp1 = m

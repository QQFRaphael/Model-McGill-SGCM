...
C DEFINE COLORS BLACK, RED, GREEN, YELLOW, BLUE, MAGENTA, CYAN & WHITE
      ...
      DATA RED   /0.,1.,0.,1.,0.,1.,0.,1./
      DATA GREEN /0.,0.,1.,1.,0.,0.,1.,1./
      DATA BLUE  /0.,0.,0.,0.,1.,1.,1.,1./
...
C
     ...
     *       y2/.97,.97,.47,.47/
        ...
        open(11,file='Z500EOF',form='unformatted')
	read (11) evt
	...
	CALL SFLUSH
	 ...
	 ii=0
         ...
         do 14 i=1,36
          ii=ii+1
14	 x(i,j)=evt(ii,k)
c	if(k.eq.1.or.k.eq.2)x(i,j)=-pot(i,j)
...
C
        CALL PCSETI ('QU - QUALITY OF CHARACTERS',1)
         ...
         ttitle=title(k)
        CALL WTSTR(x1(iic)+0.25,y2(iic)-0.01,ttitle,2,0,-1)
        CALL MAPPOS(x1(iic),x2(iic),y1(iic),y2(iic))
...
c       call cpseti ('LLP',1)
        CALL CPSETR('CIS - Contour Interval Specifier',0.02)

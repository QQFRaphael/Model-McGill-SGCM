...
C                                                                          SPECTR.4     
      COMMON/SPECTR/Z(IGB),D(IGB),T(IGB),SP(IGA),COR(IGA)
      COMPLEX Z,D,T,SP,COR
C                                                                          SPECTR.9     
C                                                                          GRIDP.2     
...
C                                                                          GRIDP.5     
     ...
     *              ,TG(IGC,NL),DG(IGC,NL),SPG(IGC),PMG(IGC),PJG(IGC)
     *              ,VGT(IGC,NL),UGT(IGC,NL),CORG(IGC)
C                                                                          LEGAU.2     
...
C*****caclulate correction term: {D} + {v}.grad(log sp)
C*****NB - only works for equally spaced levels

         DO I=1,IGC
         CORG(I)=0.
         DO L=1,NL
         CORG(I)=CORG(I) + DG(I,L)/REAL(NL) 
     &   + SECSQ(JH)*(UG(I,L)*PMG(I)+VG(I,L)*PJG(I))/REAL(NL)
         ENDDO
         ENDDO
C*****
...
C        direct Legendre and Fourier transforms
         ...
         DO 20 I=1,NT
            CALL FFT991(CORG(1+(I-1)*NCRAY*MGPP),WORK,TRIG,IFAX,1
     &                 ,MGPP,MG,NCRAY,-1)
  20     CONTINUE
         CALL FFT991(CORG(1+ NT*NCRAY*MGPP),WORK,TRIG,IFAX,1
     &              ,MGPP,MG,NRST,-1)
...
C                                                                          GRIDP.5     
     ...
     *              ,TG(IGD),DG(IGD),SPG(IGC),PMG(IGC),PJG(IGC)
     *              ,VGT(IGD),UGT(IGD),CORG(IGC)
C                                                                          COMFFT.2     
      COMMON/BATS/  BM1(IDE),AK(NNP),AQ(NL2),G(NL2),TAU(NL2)               BATS.6     
     +              ,KOUNT,KITS,KTOTAL,KRUN,BEGDAY,ITSPD,PNU21             BATS.7     
     +              ,DELT,DELT2,CV,CG,CT,PNU,PNU2                          BATS.8     
...
C                                                                          GRIDP.5     
     ...
     *              ,TG(IGL,NL),DG(IGL,NL),SPG(IGL),PMG(IGL),PJG(IGL)
     *              ,VGT(IGL,NL),UGT(IGL,NL),CORG(IGL)
      COMPLEX CHIG,SFG,UG,VG,TG,DG,SPG,PMG,PJG,VGT,UGT,CORG
...
C           VG   :   V(divergent).                                         LTIUV.41    
         ...
         CALL HEXP(T,TG  ,NL,2)
         CALL HEXP(D,DG  ,NL,2)
         CALL HEXP(SP,SPG, 1,2)
         CALL HEXP(SP,PMG, 1,2)
         CALL HEXP(SP,PJG, 1,4)
                                                                           LTIUV.47    
